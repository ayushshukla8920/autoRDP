...
DEFAULTS = {
    "host": "", "username": "", "domain": "", "port": "",
    "width": "", "height": "", "auth": "",
    "password_stdin": False, "no_input": False,
    "color": "auto", "ascii": False, "quiet": False, "verbose": 0,
    "json": False, "log_file": None, "remember": False,
    "view": None, "view_host": "127.0.0.1",
}

EPILOG = """\
examples:
  autordp                                    interactive menu
  autordp connect                            drive the session from an rdp> prompt
  autordp connect --view                     ... and watch it in a browser
...
configuration:
  Connection details come from flags, then RDP_HOST / RDP_USERNAME /
  RDP_PASSWORD / RDP_DOMAIN / RDP_PORT, then the profile saved by
  `autordp config set`. Run `autordp config env` for the full list of
  environment variables, including the input pacing knobs.
...
def _connection_parent() -> argparse.ArgumentParser:
    ...
    group.add_argument("-p", "--port", metavar="PORT", default=argparse.SUPPRESS,
...
def _view_parent() -> argparse.ArgumentParser:
    ...
    group.add_argument("--view", nargs="?", type=int, const=DEFAULT_PORT,
                       metavar="PORT", default=argparse.SUPPRESS,
                       help=f"serve a read-only live view of the remote screen "
                            f"over HTTP (default port {DEFAULT_PORT}; another "
                            f"is chosen if it is busy)")
...
def build() -> argparse.ArgumentParser:
    ...
    connect = subcommands.add_parser(
        "connect", parents=[connection, view, output],
        formatter_class=_Formatter,
    ...
    repo = subcommands.add_parser(
        "repo", parents=[connection, view, output], formatter_class=_Formatter,
    ...
    actions.add_parser("forget", parents=[output], formatter_class=_Formatter,
                       help="delete the saved profile")
