...
3. an interactive prompt, but only when there is a terminal to prompt at
...
def _working_root() -> Path:
    ...
    The current directory, and nowhere else. Every alternative is wrong now
    that this is an installed command rather than a script run from a checkout:

    * ``__file__`` lands in ``site-packages`` after ``pip install``, which is
      the wrong place to write to and is often not writable at all.
    * In a PyInstaller one-file build ``__file__`` points inside a temporary
      extraction directory that is deleted when the process exits -- so the
      STOP file would go somewhere that vanishes, and the emergency stop would
      silently never be seen.
    * ``sys.executable`` used to be the answer for the frozen build, and npm
      made it wrong: the binary now lives inside ``node_modules``, which a
      reinstall wipes and which nobody would think to look in.

    The current directory is where the user typed the command, where the
    README tells them to create the STOP file, and reliably writable.
    ``RDP_SCREENSHOT_DIR`` and ``RDP_STOP_FILE`` override it.
    """
    return Path.cwd()


PROJECT_ROOT = _working_root()

logger = logging.getLogger("autordp.config")
        ...
        if interactive:
            if saved:
                ...
                print("  (press Enter to accept a remembered value; "
                      "`autordp config set` to change them, "
                      "`autordp config forget` to clear them)")
            ...
            if not password:
                raise ConfigError(
                    ...
                    "`autordp config set --remember-password`, or run "
                    "interactively.")
