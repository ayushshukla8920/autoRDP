...
logger = logging.getLogger("autordp.input")
...
class InputSender:
    ...
    async def type_lines(self, text: str, editor_safe: bool = True,
        ...
        ``on_progress`` is called with the number of characters sent since the
        last call. Typing a repository takes tens of minutes, so a caller
        needs a finer signal than "file 3 of 40" to draw a bar that moves --
        this is that signal. Optional, and off by default: nothing in the
        typing path depends on anyone listening.
        """
        lines = text.replace("\r\n", "\n").replace("\r", "\n").split("\n")
        for index, line in enumerate(lines):
            ...
            if on_progress is not None:
                # Reported per line, not per character: a callback every 12 ms
                # would cost more than the keystroke it is reporting on, and no
                # terminal can redraw that fast anyway. The +1 accounts for the
                # newline this line just consumed.
                on_progress(len(line) + 1)
